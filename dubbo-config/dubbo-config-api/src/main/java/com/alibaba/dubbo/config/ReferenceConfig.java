...
public class ReferenceConfig<T> extends AbstractReferenceConfig {
     ...
     * 2.接着是检测泛化配置，并根据配置设置 interfaceClass 的值。
     * 3.这段逻辑用于从系统属性或配置文件中加载与接口名相对应的配置，并将解析结果赋值给 url 字段。url 字段的作用一般是用于点对点调用。
     * 4.检测几个核心配置类是否为空，为空则尝试从其他配置类中获取。
     * 5.收集各种配置，并将配置存储到 map 中。
     * 6.处理 MethodConfig 实例。该实例包含了事件通知配置，比如 onreturn、onthrow、oninvoke 等。
     * 7.解析服务消费者 ip，以及调用 createProxy 创建代理对象。
     */
    private void init() {
        // 避免重复初始化
        if (initialized) {
     ...
     * 创建远程方法的代理
     *
     * 首先根据配置检查是否为本地调用，若是，则调用 InjvmProtocol 的 refer 方法生成 InjvmInvoker 实例。若不是，则读取直连配置项，
     * 或注册中心 url，并将读取到的 url 存储到 urls 中。
     * 然后根据 urls 元素数量进行后续操作。若 urls 元素数量为1，则直接通过 Protocol 自适应拓展类构建 Invoker 实例接口。
