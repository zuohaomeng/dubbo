...
public class RegistryDirectory<T> extends AbstractDirectory<T> implements NotifyListener {
    ...
    private void refreshInvoker(List<URL> invokerUrls) {
        ...
        if (invokerUrls != null && invokerUrls.size() == 1 && invokerUrls.get(0) != null
            ...
            // 将 newUrlInvokerMap 转成方法名到 Invoker 列表的映射  key:methodName,invoke列表
            //methodInvokerMap 变量在 doList 方法中会被用到
     ...
     * 紧接着是合并 url，然后访问缓存，尝试获取与 url 对应的 invoker。如果缓存命中，直接将 Invoker 存入 newUrlInvokerMap 中即可。如果未命中，则需新建 Invoker。
    ...
    /**
     * Invoker 列举逻辑
     * 一般情况下，普通 的调用可通过方法名获取到对应的 Invoker 列表，泛化调用可通过 ***** 获取到 Invoker 列表。
